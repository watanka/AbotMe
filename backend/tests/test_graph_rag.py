...
#     name="test",
#     email="test",
#     pdf_url="./eval/eval-resume.pdf",
# )
# run_resume_pipeline(resume, llm, uow)
# run_graph_resume_pipeline(resume, llm, graph_db, uow)


# load test data
with open("eval/test_dataset.json", "r") as f:
    test_data = json.load(f)["test_dataset"]

graph_rag_dataset = []
rag_dataset = []

# for data in test_data:
#     graph_rag_retrieved_contexts = graph_rag.retrieve_context(data["question"])
#     rag_retrieved_contexts = rag.retrieve_context(data["question"])

#     graph_rag_answer = graph_rag.generate_answer(
#         data["question"], graph_rag_retrieved_contexts
#     )
#     rag_answer = rag.generate_answer(data["question"], rag_retrieved_contexts)

#     graph_rag_dataset.append(
#         {
#             "user_input": data["question"],
#             "retrieved_contexts": graph_rag_retrieved_contexts,
#             "response": graph_rag_answer,
...
#         {
#             "user_input": data["question"],
#             "retrieved_contexts": rag_retrieved_contexts,
#             "response": rag_answer,
#             "reference": data["ground_truth"],
#         }
#     )
...
data = {
    "question": "Dynamic batching을 구현하기 위해 사용한 클래스는 무엇인가요?",
    "ground_truth": ["Bucket 클래스", "Monitor 클래스"],
    "tags": ["side_project", "technical_detail", "implementation"],
}
...
graph_rag_evaluation_dataset = EvaluationDataset(graph_rag_dataset)
rag_evaluation_dataset = EvaluationDataset(rag_dataset)


evaluator_llm = LangchainLLMWrapper(llm)


graph_rag_result = evaluate(
    dataset=graph_rag_evaluation_dataset,
